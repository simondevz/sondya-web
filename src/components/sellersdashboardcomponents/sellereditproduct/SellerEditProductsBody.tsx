...
import { SELLER_UPDATE_PRODUCT_RESET } from "../../../redux/constants/seller/seller-products.constants";
...
const SellerEditProductsBody = () => {
  ...
  useEffect(() => {
    if (sellerGetProductByIDRedux?.serverResponse.data) {
      ...
      if (
        ...
        sellerGetProductByIDRedux?.serverResponse?.data.image.forEach((image) =>
  ...
  const sellerUpdateProductRedux = useSelector(
    (state: ReducersType) => state?.sellerUpdateProduct
  ) as ReduxResponseType;

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (formData) {
      dispatch(sellerUpdateProductAction(formData, deleteImageId) as any);
    }
  };

  useEffect(() => {
    sellerUpdateProductRedux?.error &&
      Swal.fire({
        icon: "error",
        title: "Oops...",
        timer: 5000,
        text: sellerUpdateProductRedux?.error,
      });
    sellerUpdateProductRedux?.error && setDeleteImageId([]);
    sellerUpdateProductRedux?.success &&
      Swal.fire({
        icon: "success",
        title: "Successful",
        timer: 5000,
        text: sellerUpdateProductRedux?.serverResponse?.message,
      });
    if (sellerUpdateProductRedux?.success) {
      setTimeout(function () {
        dispatch(sellerGetProductByIdAction(id as string) as any);
      }, 1000);
      setTimeout(() => {
        // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <section>
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        >
          {sellerUpdateProductRedux?.loading ? (
